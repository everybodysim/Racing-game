...
## Where to go from here (after this repo update)

1. Open your game page and confirm the new **Live Chat** panel appears on the right-middle side.
2. Drag the corner of the chat panel to resize it to your preference.
3. Type in the chat box and press **Enter** (or tap **Send**).
4. Open the same track on another device/tab and confirm messages appear within a few seconds.
5. If messages do not show:
   - verify your deployed Worker has the `CHAT_KV` binding,
   - verify your site can reach `/api/chat/messages`,
   - check that your Cloudflare route maps `/api/chat/*` to the chat Worker.

If you deploy the chat Worker on a different domain instead of `/api`, set `CHAT_API_BASE` in `js/main.js` to that full URL.
