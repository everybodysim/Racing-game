...
export function buildWallColliders( world, debugGroup, customCells, extras = null ) {
	...
	const ELEVATED_SURFACE_HALF_H = 0.14 * S;
	const ELEVATED_SURFACE_HALF_XZ = CELL_HALF * S * 1.14;
	const ELEVATED_SURFACE_DROP = 0.4;
	const SLOPE_SURFACE_HALF_XZ = ELEVATED_SURFACE_HALF_XZ + ( 0.015 * S );
	const SLOPE_SURFACE_HALF_H = ELEVATED_SURFACE_HALF_H + ( 0.015 * S );
	const SLOPE_SURFACE_ANGLE_BIAS = THREE.MathUtils.degToRad( 0.6 );
	...
	function addSlopeCollider( gx, gz, orient = 0, up = true ) {
		...
		const slopePitch = ( up ? slopeAngle : - slopeAngle ) + ( up ? SLOPE_SURFACE_ANGLE_BIAS : - SLOPE_SURFACE_ANGLE_BIAS );
		const quat = new THREE.Quaternion().setFromEuler( new THREE.Euler( slopePitch, yaw, 0, 'YXZ' ) );
		const halfExtents = [ SLOPE_SURFACE_HALF_XZ, SLOPE_SURFACE_HALF_H, SLOPE_SURFACE_HALF_XZ ];
