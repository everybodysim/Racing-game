...
export function buildWallColliders( world, debugGroup, customCells, extras = null ) {
	...
	const ELEVATED_SLOPE_SEAM_HALF_LONG = CELL_HALF * S * 0.68;
	const ELEVATED_SEAM_SINK = 0.02 * S;
	...
	function addElevatedSeamConnector( gxA, gzA, entryA, gxB, gzB, entryB ) {
		...
		const isSlopeA = entryA.type === 'slope-up';
		const isSlopeB = entryB.type === 'slope-up';
		const isSlopeTransition = isSlopeA !== isSlopeB;
		const slopePose = isSlopeA ? poseA : ( isSlopeB ? poseB : null );
		const midY = isSlopeTransition && slopePose
			? getSurfaceYAtXZ( slopePose, midX, midZ ) - ELEVATED_SEAM_SINK
			: Math.min( yA, yB ) - ELEVATED_SEAM_SINK;

		const quaternion = isSlopeTransition && slopePose
			? slopePose.quat.clone()
			: poseA.quat.clone().slerp( poseB.quat, 0.5 );
		const dx = gxB - gxA;
		const dz = gzB - gzA;
		const seamHalfAlongX = Math.abs( dx ) === 1
			? ( isSlopeTransition ? ELEVATED_SLOPE_SEAM_HALF_LONG : ELEVATED_SEAM_HALF_NARROW )
			: ELEVATED_SEAM_HALF_WIDE;
		const seamHalfAlongZ = Math.abs( dz ) === 1
			? ( isSlopeTransition ? ELEVATED_SLOPE_SEAM_HALF_LONG : ELEVATED_SEAM_HALF_NARROW )
			: ELEVATED_SEAM_HALF_WIDE;
		const halfExtents = [ seamHalfAlongX, ELEVATED_SEAM_HALF_H, seamHalfAlongZ ];
