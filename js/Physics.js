...
export function buildWallColliders( world, debugGroup, customCells, extras = null ) {
	...
	const ELEVATED_SEAM_BUMP_HALF_SPAN = CELL_HALF * S * 0.95;
	const ELEVATED_SEAM_BUMP_RISE = 0.028 * S;
	const ELEVATED_SEAM_BUMP_RADIUS = ( ELEVATED_SEAM_BUMP_HALF_SPAN * ELEVATED_SEAM_BUMP_HALF_SPAN + ELEVATED_SEAM_BUMP_RISE * ELEVATED_SEAM_BUMP_RISE ) / ( 2 * ELEVATED_SEAM_BUMP_RISE );
	...
	function addElevatedSeamConnector( gxA, gzA, entryA, gxB, gzB, entryB ) {
		...
		const position = [ midX, elevatedSurfaceY + ELEVATED_SEAM_BUMP_RISE - ELEVATED_SEAM_BUMP_RADIUS, midZ ];

		rigidBody.create( world, {
			shape: sphere.create( { radius: ELEVATED_SEAM_BUMP_RADIUS } ),
			motionType: MotionType.STATIC,
			objectLayer: world._OL_STATIC,
			position,
			friction: 1.0,
		...
		if ( debugGroup ) addDebugSphere( debugGroup, ELEVATED_SEAM_BUMP_RADIUS, position );
