...
export class AdvancementManager {
  constructor(events,opts={}){ this.events=events; this.state=opts.state||{}; this.onUnlock=opts.onUnlock||(()=>{}); this.accountDirtyRef=opts.accountDirtyRef||{value:false}; this.now=opts.now||(()=>performance.now()/1000); this.session={resets:0,driftStart:null,topSpeed:0,crashed:false}; this.bind(); }
  ...
  bind(){ this.events.on('race_finished',(p)=>{this.unlock('first_race'); if(p?.crashed===false) this.unlock('clean_finish');}); this.events.on('drift_started',()=>{this.session.driftStart=this.now(); this.unlock('first_drift');}); this.events.on('drift_ended',()=>{ if(this.session.driftStart!=null){ const d=this.now()-this.session.driftStart; if(d>=10) this.unlock('drift_10s'); this.session.driftStart=null; }});
