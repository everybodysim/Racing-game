...
actionDefDropdown( 'action_set_vehicle_model', 'set vehicle model', 'MODEL', [ [ 'Yellow Truck', 'vehicle-truck-yellow' ], [ 'Green Truck', 'vehicle-truck-green' ], [ 'Purple Van', 'vehicle-truck-purple' ], [ 'Red Truck', 'vehicle-truck-red' ], [ 'Green Hatchback', 'vehicle-hatchback-green' ], [ 'Orange Sedan', 'vehicle-sedan-orange' ] ], null, null, VEH );
...
function generateTemplate() {
	...
	const stub = `( { id: ${ JSON.stringify( id ) }, init(){}, applyFrame(){ return null; }, onCheckpoint(){}, onCrash(){}, onRespawn(){}, onLapFinish(){}, dispose(){} } )`;
	return `// ${ name }\nconst SPEC = ${ JSON.stringify( spec, null, 2 ) };\nconst R = window.__RACING_MOD_RUNTIME__ || ( window.__RACING_MOD_RUNTIME__ = {} );\nexport default ( R.createRuntime || ( R.createRuntime = () => ${ stub } ) )( ${ JSON.stringify( id ) }, SPEC );\n`;
}

// Verbose, self-contained variant (inlines the full interpreter). Used ONLY for
// the "Generate JS Template" button so advanced users can copy a standalone
// runnable module. Never stored to localStorage - that's the compact form above.
function generateVerboseTemplate() {
	const id = safeId( document.getElementById( 'mod-id' )?.value ) || `custom-${ Date.now() }`;
	const name = ( document.getElementById( 'mod-name' )?.value || 'Custom Mod' ).trim();
	const spec = buildRuntimeSpec();
...
document.getElementById( 'export-template' )?.addEventListener( 'click', () => {
	document.getElementById( 'xmlBox' ).value = generateVerboseTemplate();
