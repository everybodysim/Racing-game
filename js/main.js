...
async function init() {
	...
	function updateStuntPointsHud() {
		...
		stuntPointsHud.innerHTML = `Points: ${ Math.floor( stuntPoints ) }<small>Bonus: ${ stuntReasonText }</small>`;
	...
	function animate() {
			...
			if ( hasLeftStartZone && allCheckpointsPassed && crossedFinish ) {
				...
				for ( const checkpoint of checkpointStates ) {
					...
					if ( gameMode === 'stunt' ) {
						...
						stuntReasonText = '--';
						stuntReasonTimer = 0;
						if ( lapBonus > 0 ) addStuntPoints( lapBonus, `Fast lap +${ lapBonus}`, 1.6 );
						updateStuntPointsHud();
		...
		if ( gameMode === 'stunt' ) {
			...
			const overspeed = speedRatio > 1.02;
			...
			if ( vehicle.driftIntensity > 0.65 ) addStuntPoints( ( vehicle.driftIntensity - 0.65 ) * 34 * dt, 'Drift' );
			if ( overspeed && hasBoostSource ) addStuntPoints( 28 * dt, 'Speed burst' );
			if ( isAirborne ) addStuntPoints( 30 * dt, vehicle.spherePos.y > 1.6 ? 'Big jump' : 'Air' );
