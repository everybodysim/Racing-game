...
const MAGNET_MAX_DISTANCE_BLOCKS = 2.5;
const MAGNET_FORCE_PER_SECOND = 26.0;
...
async function init() {
	...
	const magnetMaxDistance = CELL_RAW * GRID_SCALE * MAGNET_MAX_DISTANCE_BLOCKS;
	const magnetEntries = magnetCells
		.map( ( [ gxRaw, gzRaw, yGridRaw, variant ] ) => {
	...
	function applyMagnetForceFor( targetVehicle, dt ) {
		...
		for ( const magnet of magnetEntries ) {
			...
			if ( distance <= 1e-4 || distance > magnetMaxDistance ) continue;
			...
			if ( distance <= magnetFullStrengthDistance ) strengthScale = 1;
				...
				const t = THREE.MathUtils.clamp(
					( distance - magnetFullStrengthDistance ) / Math.max( 1e-6, magnetMaxDistance - magnetFullStrengthDistance ),
					0,
					1
				);
			...
			const impulse = MAGNET_FORCE_PER_SECOND * strengthScale * dt;
