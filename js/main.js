...
const TRACK_SHARE_API_BASE = 'https://racing-track-board-api.ga1010.workers.dev/api';
...
async function resolvePackedTrackParams( params ) {
	...
	try {
		...
		const response = await fetch( `${ TRACK_SHARE_API_BASE }/packs/${ encodeURIComponent( packId ) }`, { cache: 'no-store' } );
		if ( ! response.ok ) throw new Error( `pack-http-${ response.status }` );
		const payload = await response.json();
		if ( ! payload?.ok ) throw new Error( 'pack-invalid-response' );
		return {
...
async function init() {
	...
	async function fetchCampaignTracks() {
		...
		try {
			...
			const response = await fetch( `${ TRACK_SHARE_API_BASE }/tracks` );
			if ( ! response.ok ) return [];
