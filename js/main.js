...
function getLegacyTrackIds( mapParamValue, extrasParamValue ) {
	...
	return [ ...new Set( ids.filter( ( id ) => Boolean( id ) && id.length <= 512 ) ) ];
...
async function init() {
	...
	try {
			...
			const settled = await Promise.allSettled( trackIdsToRead.map( async ( trackId ) => {
			...
			const payloads = settled.filter( ( entry ) => entry.status === 'fulfilled' ).map( ( entry ) => entry.value );
			if ( payloads.length === 0 ) throw new Error( 'no-leaderboard-sources-succeeded' );
