...
async function init() {
	...
	function animate() {
		...
		if ( fxSettings.motionBlur ) {
			...
			const speedBlur = THREE.MathUtils.clamp( Math.abs( vehicle.linearSpeed ) * 0.016, 0, 0.16 );
			const impactBlur = raceClockSeconds < impactBlurUntil ? 0.22 : 0;
			bloomPass.strength = 0.02 + speedBlur + impactBlur;
			bloomPass.radius = 0.02 + speedBlur * 0.9 + impactBlur * 0.8;
