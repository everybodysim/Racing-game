...
async function init() {
	...
	const finishData = (() => {
		...
		const lapDirection = orient === 10 || orient === 22 ? - 1 : 1;

		return { centerX, centerZ, halfExtent, angle, cosA, sinA, lapDirection };
	...
	function animate() {
		...
		if ( finishData ) {
			...
			const crossedFinish = finishData.lapDirection > 0 ?
				( lastLocalZ <= - 0.02 && localZ > 0.02 ) :
				( lastLocalZ >= 0.02 && localZ < - 0.02 );
