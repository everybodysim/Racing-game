...
async function init() {
	...
	function ensureRemotePlayerVisual( playerId ) {

		if ( remotePlayerVisuals.has( playerId ) ) return remotePlayerVisuals.get( playerId );
		...
		scene.add( mesh );
		remotePlayerVisuals.set( playerId, mesh );
		return mesh;
	...
	function removeRemotePlayerVisual( playerId ) {
		...
		const mesh = remotePlayerVisuals.get( playerId );
		if ( ! mesh ) return;
		scene.remove( mesh );
	...
	async function syncMultiplayerTransforms() {
		...
		try {
			...
			for ( const [ playerId, playerState ] of Object.entries( players ) ) {
				...
				const visual = ensureRemotePlayerVisual( playerId );
				visual.position.set( Number( playerState?.x ) || 0, ( Number( playerState?.y ) || 0 ) + 0.28, Number( playerState?.z ) || 0 );
				visual.rotation.set( 0, Number( playerState?.ry ) || 0, 0 );
				seen.add( playerId );
