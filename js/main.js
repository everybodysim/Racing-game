...
async function init() {
	...
	try {
		...
		if ( Number.isFinite( Number( parsed?.recentGhostCount ) ) ) fxSettings.recentGhostCount = THREE.MathUtils.clamp( Math.round( Number( parsed.recentGhostCount ) ), 1, 5 );
	...
	<label style="display:block;margin-top:4px;">Recent ghost count <input id="fx-recent-ghost-count" type="number" min="1" max="5" step="1" value="${ fxSettings.recentGhostCount }" style="width:100%;margin-top:3px;background:#0f1520;color:#e9f5ff;border:1px solid rgba(255,255,255,0.25);border-radius:6px;padding:2px 4px;"></label>`;
	...
	fxRecentGhostCountSelect?.addEventListener( 'change', () => {
		...
		if ( Number.isFinite( value ) ) fxSettings.recentGhostCount = THREE.MathUtils.clamp( Math.round( value ), 1, 5 );
	...
	function updateGaragePaletteUi() {
		...
		const renderPalette = ( container, entries ) => {
			...
			for ( const entry of entries ) {
				...
				button.title = unlockedPaints[ entry.id ] ? `Select ${ entry.hex }` : `Unlock ${ entry.hex } (${ entry.unlockCost } coins)`;
				button.addEventListener( 'click', () => {
					...
					if ( unlockedPaints[ entry.id ] ) {
						...
						updateGaragePaletteUi();
						setGarageMappingStatus( `Selected ${ entry.finish === 'shiny' ? 'shiny' : 'paint' } ${ entry.hex }.` );
						return;

					}
					if ( coins < entry.unlockCost ) {

						setGarageMappingStatus( `Need ${ entry.unlockCost } coins to unlock this ${ entry.finish === 'shiny' ? 'shiny paint' : 'paint' }.`, true );
						return;

					}
					coins -= entry.unlockCost;
					...
					updateEconomyHud();
					updateGarageUi();
					setGarageMappingStatus( `Unlocked ${ entry.finish === 'shiny' ? 'shiny paint' : 'paint' } ${ entry.hex }.` );
