...
async function init() {
	...
	function ensureRemotePlayerVisual( playerId, carKey ) {
		...
		const modelKey = typeof carKey === 'string' && models[ carKey ] ? carKey : 'vehicle-truck-yellow';
		...
		scene.add( mesh );
		const state = { mesh, carKey: modelKey, targetPos: mesh.position.clone(), targetRotY: mesh.rotation.y };
		...
		try {
			...
			for ( const [ playerId, playerState ] of Object.entries( players ) ) {
				...
				if ( now - updatedAt > REMOTE_PLAYER_STALE_MS ) continue;
					...
					visualState.targetPos.set( Number( playerState?.x ) || 0, Number( playerState?.y ) || 0, Number( playerState?.z ) || 0 );
					visualState.targetRotY = ( Number( playerState?.ry ) || 0 ) + Math.PI;
					seen.add( playerId );
