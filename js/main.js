...
async function init() {
	...
	function buildGarageShinyPalette() {
		...
		for ( let i = 0; i < gradientHues.length; i ++ ) {

			const [ startHex, endHex ] = gradientHues[ i ];
			const start = new THREE.Color( startHex );
			const end = new THREE.Color( endHex );
			const mid = start.clone().lerp( end, 0.5 );
			colors.push( {
				id: `sg-${ i }`,
				hex: `#${ mid.getHexString() }`,
				preview: `linear-gradient(135deg, ${ startHex }, ${ endHex })`,
				row: 1,
				col: i,
				unlockCost: GARAGE_SHINY_UNLOCK_COST,
				finish: 'shiny',
	...
	function updateGaragePaletteUi() {
		...
		const renderPalette = ( container, entries ) => {
			...
			for ( const entry of entries ) {
				...
				button.style.background = entry.preview || entry.hex;
