...
async function init() {
	...
	function cloneRemoteCarModel( carKey ) {

		const sourceModel = models[ carKey ] || models[ 'vehicle-truck-yellow' ];
		if ( ! sourceModel ) return null;
		const remoteModel = sourceModel.clone( true );
		remoteModel.traverse( ( obj ) => {
			...
			obj.material = materials.map( ( mat ) => {
				...
				cloned.opacity = Math.min( 0.48, cloned.opacity ?? 1 );
				cloned.depthWrite = false;
				if ( cloned.color ) cloned.color.lerp( new THREE.Color( 0x53d4ff ), 0.35 );
				return cloned;
	...
	function ensureRemotePlayerVisual( playerId, carKey ) {
		...
		if ( existing && existing.carKey === carKey ) return existing;
		if ( existing ) removeRemotePlayerVisual( playerId );
		const mesh = cloneRemoteCarModel( carKey ) || new THREE.Mesh(
		...
		scene.add( mesh );
		const state = { mesh, carKey: carKey || 'vehicle-truck-yellow', targetPos: mesh.position.clone(), targetRotY: mesh.rotation.y };
		...
		try {
			...
			for ( const [ playerId, playerState ] of Object.entries( players ) ) {
				...
				if ( now - updatedAt > REMOTE_PLAYER_STALE_MS ) continue;
					...
					visualState.targetPos.set( Number( playerState?.x ) || 0, ( Number( playerState?.y ) || 0 ) + 0.28, Number( playerState?.z ) || 0 );
