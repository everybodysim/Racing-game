...
async function init() {
	...
	function updateModeHudVisibility() {
		...
		if ( lapHud ) lapHud.style.display = inStunt ? 'none' : 'block';
		if ( lapHud2 ) lapHud2.style.display = ! inStunt && isSplitScreen ? 'block' : 'none';
		if ( economyHud && ! isSplitScreen ) economyHud.style.display = inStunt ? 'none' : 'block';
		if ( shareTimeBtn ) shareTimeBtn.style.display = ! inStunt && ! isSplitScreen ? 'block' : 'none';
		if ( exportGhostBtn ) exportGhostBtn.style.display = ! inStunt && ! isSplitScreen ? 'block' : 'none';
		if ( importGhostBtn ) importGhostBtn.style.display = ! inStunt && ! isSplitScreen ? 'block' : 'none';
	...
	function updateStuntPointsHud() {
		...
		stuntPointsHud.textContent = `Points: ${ Math.floor( stuntPoints ) }`;
	...
	function animate() {
		...
		if ( gameMode === 'race' ) for ( const checkpoint of checkpointStates ) {
		...
		if ( gameMode === 'race' && finishData && vehicle2 ) {
		...
		if ( gameMode === 'race' ) {
			...
			const isAirborne = vehicle.spherePos.y > 0.9 || Math.abs( vehicle.sphereVel.y ) > 1.4;
			if ( vehicle.driftIntensity > 0.65 ) stuntPoints += ( vehicle.driftIntensity - 0.65 ) * 34 * dt;
			if ( speedRatio >= 0.97 && Math.abs( input.z ) > 0.2 ) stuntPoints += 26 * dt;
			if ( isAirborne ) stuntPoints += 30 * dt;
			updateStuntPointsHud();
