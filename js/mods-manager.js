...
function renderShared(mods) { const list=document.getElementById('shared-mods-list'); if(!list) return; list.innerHTML=''; for(const mod of mods){ const li=document.createElement('li'); li.textContent=`${mod.modName||mod.modId} (${mod.modId})`; const install=document.createElement('button'); install.textContent='Install'; install.style.marginLeft='8px'; install.disabled=!CUSTOM_MODS_ENABLED; install.title=CUSTOM_MODS_ENABLED?'':'Custom mod install is under construction'; install.onclick=()=>{ if(!CUSTOM_MODS_ENABLED) return; const code = String(mod.template||''); if(!code.trim()) return; const installed = readInstalled(); const id = `custom-${mod.modId||Date.now()}`; const next = installed.filter((m)=>m.id!==id); next.push({ id, name: mod.modName||mod.modId||'Custom Shared Mod', entry: toBase64JsDataUrl(code) }); saveInstalled(next); renderInstalled(next); renderCatalog(currentCatalog,next); }; const copy=document.createElement('button'); copy.textContent='Copy JSON'; copy.style.marginLeft='8px'; copy.onclick=()=>navigator.clipboard?.writeText(JSON.stringify(mod)); const rm=document.createElement('button'); rm.textContent='Delete'; rm.style.marginLeft='8px'; rm.onclick=()=>{const n=readSharedMods().filter((x)=>x.modId!==mod.modId); saveSharedMods(n); renderShared(n);}; li.append(install,copy,rm); list.appendChild(li);} }
...
(async function init(){
  ...
  if (!CUSTOM_MODS_ENABLED) status.textContent='Custom mods are under construction and temporarily disabled.';
  ...
  document.getElementById('import-shared-btn')?.addEventListener('click',()=>{ if(!CUSTOM_MODS_ENABLED){ status.textContent='Custom mods are under construction. Import is disabled for now.'; return; } try{ const payload=parseSharedInput(document.getElementById('shared-mod-input')?.value); if(payload.type!=='racing-custom-mod-share-v1') throw new Error('Invalid payload type'); const all=readSharedMods().filter((x)=>x.modId!==payload.modId); all.push(payload); saveSharedMods(all); renderShared(all); status.textContent=`Imported shared mod ${payload.modName||payload.modId}`;}catch(e){ status.textContent=`Import failed: ${e.message}`; }});
})();
