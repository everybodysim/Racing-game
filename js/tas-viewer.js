...
dir.position.set(11.4, 15, -5.3);
scene.add(dir);
scene.add(new THREE.HemisphereLight(0xc8d8e8, 0x7a8a5a, 1.6));
...
function updateCarConfig() {
  ...
  vehicle.setPerformance(CAR_STATS[carKey] || CAR_STATS['vehicle-truck-yellow']);
}
...
carSelect?.addEventListener('change', () => {
  updateCarConfig();
  statusEl.textContent = 'TAS car config updated.';
});

document.getElementById('export-btn')?.addEventListener('click', async () => {
  const code = encodeCode({ steps: parseInputLines(inputsEl.value), trackUrl: trackUrlInput.value.trim(), car: carSelect.value });
  try {
