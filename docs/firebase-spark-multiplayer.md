...
- Gameplay multiplayer uses Firebase Web SDK `onValue` listeners for `/players` and `/lapTimes`, plus `onDisconnect().remove()` for each player row; REST remains only as a fallback if the SDK cannot initialize.
...
- Editor rooms also prefer SDK listeners so connected editor pages apply remote track revisions as they arrive, with REST polling retained as a fallback.
